...
def features2(sent, context_size=2, undefined='__'):
    ...
    the sentence, its lemma, POS, whether the first letter is uppercase (isupperfirst), whether all letter are uppercased (isupper), the length of the token, and third level hypernym of the first Wordnet synset of the first hypernyms as well as that of the two
    ...
    },
    {
      ...
      "2:length": 2,
      "2:pos": "VERB",
      "2:wnhypernym3": "Synset('substance.n.01')"
    },
    ...
    for i, token in enumerate(sent):
        ...
        for j in range(-context_size, context_size + 1):
            ...
            if 0 <= k < len(sent):
                ...
                if str(sent[k]).isupper():
                    isupper = str(sent[k]).isupper()
                else:
                    isupper = False
                    isupperfirst = str(sent[k])[0].isupper() 
                length = len(str(sent[k]))
            else:
                lemma = pos = wnhypernym3 = isupper = isupperfirst = length = undefined
                

            token_feats['{}:lemma'.format(j)] = lemma
            token_feats['{}:pos'.format(j)] = pos
            token_feats['{}:wnhypernym3'.format(j)] = wnhypernym3
            token_feats['{}:isupper'.format(j)] = isupper
            token_feats['{}:isupperfirst'.format(j)] = isupperfirst
            token_feats['{}:length'.format(j)] = length
